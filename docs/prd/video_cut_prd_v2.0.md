...
*   **处理逻辑**：
    ...
    5.  **智能多维筛选算法（Smart Filtering）**：
        *   **目标**：从海量识别结果中，精准识别出老师正在**重点讲解**的单词，过滤掉随口一提的辅助词。
        *   **核心原理**：结合频率密度、上下文语义引导和语言环境进行多维度评分。
        *   **评分规则**：
            1.  **基础分**：单词每出现一次，得 **1分**。
            2.  **密度分（Density Score）**：如果在短时间窗口（120秒）内高频出现（≥3次），每次出现额外得 **2分**。
            3.  **上下文分（Context Score）**：如果单词周围出现教学引导词（如"这个单词是"、"意思是"、"叫做"、"翻译成"），每次出现额外得 **5分**。
            4.  **独立性分（Isolation Score）**：如果英文单词被中文包围（中英混合模式），说明它是被解释的对象，每次出现额外得 **2分**。
        *   **筛选阈值**：总分 ≥ **15分** 的单词被保留。
        *   **优势**：
            *   能找回低频但重要的词（如老师只拼写了一次 "s-i-s-t"）。
            *   能过滤掉高频但无意义的词（如反复说的 "ok", "so"）。
...
*   **数据结构优化**：
    *   支持多 P 视频（B站的分集视频）：需记录 `BVID` + `Page_Index (page)`。

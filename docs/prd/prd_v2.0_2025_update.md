...
#### 1.1 文库列表
*   **入口**：主导航栏新增"文库"入口。
*   **展示方式**：**列表式布局** (List Layout)，每个文章占据一行，提供更紧凑的视图和更高的信息密度。
*   **展示项**：
    *   📖 **文章图标**：视觉标识
    *   **文章标题** (Title)：主标题，h6 字体
    *   **难度等级** (Difficulty Level)：L1/L2/L3 彩色标签
    *   **生成时间** (Created Time)：格式化日期
    *   **核心词汇预览** (Target Words)：显示前3个单词，多余显示"+N"
    *   📊 **测试次数** (Quiz Count)：显示该文章已被测试过几次（例如："3 quizzes"）
    *   🏆 **最高分数** (Highest Score)：显示历史最佳成绩（例如："Best: 85%"），仅在有测试记录时显示
*   **操作**：
    *   **阅读** (Read)：点击进入阅读页。
    *   **删除** (Delete)：从文库中移除（同时级联删除相关的测试记录）。
*   **布局特点**：
    *   单列列表，每行显示完整文章信息
    *   悬停效果：鼠标悬停时背景高亮
    *   分隔线：文章之间有清晰分隔

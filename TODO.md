...
## [Feature] Quiz Review & Explanation
- [x] Task 1 (States): Refactor `ReadingPage.tsx` state machine to support `'review'` mode.
- [x] Task 2 (Entry): Add "Review Answers" button to `ScoreFeedback.tsx`.
- [x] Task 3 (UI): Update `QuizView.tsx` to display explanations when in `readOnly` mode.
- [x] Task 4 (Nav): In `QuizView` (review mode), change "Submit" to "Exit Review".
- [x] Task 5 (Data): Ensure `explanation` field is correctly passed.
